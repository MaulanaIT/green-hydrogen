...
export class GreenHydrogenIndonesiaInitiationAction extends Component {
    ...
    componentDidMount() {
        axios.get(`https://admin.greenhydrogen.my.id/api/content/${i18next.language}/menu/${i18next.language == 'id' ? 29 : 30}`, ConfigAPI()).then(response => {
            ...
            if (data) {
                data.map((item, index) => {
                    if (item.order == '0') {
                        htmlContent.push(
                            <div key={index} className="fluid-section-one" style={{ background: `url(${StorageURL(item?.content?.background_initiation_picture_1)}) no-repeat`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
...
export default GreenHydrogenIndonesiaInitiationAction
