...
export class GreenHydrogenIndonesiaLegalFramework extends Component {
    ...
    componentDidMount() {
        axios.get(`https://admin.greenhydrogen.my.id/api/content/${i18next.language}/menu/${i18next.language == 'id' ? 25 : 26}`, ConfigAPI()).then(response => {
            ...
            if (data) {
                data.map((item, index) => {
                    if (item.order == '0') {
                        htmlContent.push(
                            <div key={index} className="fluid-section-one" style={{ background: `url(${StorageURL(item?.content?.background_legal_picture_1)}) no-repeat`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                                        ...
                                        <div className="content-column">
                                            <div className="inner-column">
                                                <div>
                                                    ...
                                                    <p className='text-center text-lg-start' dangerouslySetInnerHTML={{ __html: item?.content?.left_description_2 }}></p>
                                                </div>
                                            </div>
                                        </div>
                                        {/*  Content Column */}
...
export default GreenHydrogenIndonesiaLegalFramework
